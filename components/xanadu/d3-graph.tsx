...
function NodeAvatar({ pubkey, nodeId }: { pubkey: string; nodeId: string }) {
  ...
  useEffect(() => {
    if (profile?.picture) {
      // Update the node's avatar when profile is loaded
...
export function D3Graph({ width, height }: D3GraphProps) {
  ...
  }, [nodes.length, connections.length, selectedNodeId, width, height])
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {d3Nodes
        ...
        .map((node) => (
          <NodeAvatar key={node.id} pubkey={node.pubkey!} nodeId={node.id} />
        ))}
